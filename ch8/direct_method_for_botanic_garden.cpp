...
// paths
string left_file = "./1666059839850277901.png";
string depth_file = "./1666059839850277901.tiff";
...
void read_image_names_from_directory(const std::string& path, std::vector<std::string>& image_names) {
    ...
    while ((entry = readdir(dir)) != nullptr) {
        ...
        // You may add filtering here, e.g., only ".png", ".jpg", etc.
...
int main(int argc, char **argv) {
    ...
    std::vector<std::string> lrgb_names;
    read_image_names_from_directory("botanic_garden_segment", lrgb_names);
    ...
    for ( int x=10; x<left_img.cols-10; x++ ){
        for ( int y=10; y<left_img.rows-10; y++ ){
            ...
            if ( delta.norm() < 100 )
                continue;
            double depth =  double( depth_img.ptr<float>(y)[x] );
            if(depth <= 0)
                continue;
    ...
    for (size_t i=0; i < lrgb_names.size(); i++) {  // 1~10
        cv::Mat curr_lrgb = cv::imread("botanic_garden_segment/" + lrgb_names.at(i), cv::IMREAD_COLOR);
        cv::Mat curr_lgray;
        cv::cvtColor(curr_lrgb, curr_lgray, cv::COLOR_BGR2GRAY);
        // try single layer by uncomment this line
        // DirectPoseEstimationSingleLayer(left_img, img, pixels_ref, depth_ref, T_cur_ref, true);
        DirectPoseEstimationMultiLayer(left_img, curr_lgray, pixels_ref, depth_ref, T_cur_ref);
    }
    return 0;
}
